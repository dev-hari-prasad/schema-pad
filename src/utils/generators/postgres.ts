...
export function generatePostgres(tables: TableNode[], relationships: Relationship[]): string {
  ...
  for (const table of tables) {
    ...
    for (const rel of fkRels) {
      ...
      if (srcCol && tgtTable && tgtCol) {
        colLines.push(`  FOREIGN KEY ("${srcCol.name}") REFERENCES "${tgtTable.name}" ("${tgtCol.name}")`);
      }
    }
