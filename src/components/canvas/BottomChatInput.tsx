...
const SUGGESTIONS = [
  ...
  "Normalize my tables",
  "Optimize relationships for queries",
  "What's missing in my schema?",
  "How can I improve my schema?",
  "Enhance my schema for scalability",
];
...
export const BottomChatInput = () => {
  ...
  useEffect(() => {
    const handleAskAI = (event: Event) => {
      ...
      if (!message) return;
      setChatMessage({ text: message, tick: Date.now() });
      setIsOpen(true);
      setValue("");
    };
    window.addEventListener('schema:ask-ai-from-command', handleAskAI as EventListener);
  ...
  return (
    <div
      ...
      className={cn(
        ...
        dockRight ? 'bottom-3 right-3 items-end' : 'bottom-4 left-1/2 -translate-x-1/2 items-center'
      )}
    >
      <motion.div
        initial={false}
      ...
      <div className="flex items-end gap-2">
        {dockRight && (
          ...
          >
            ...
            <div className="absolute bottom-full left-0 mb-2 w-60 p-3 rounded-lg bg-popover text-popover-foreground border border-border shadow-xl opacity-0 translate-y-2 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-200 z-50">
        ...
        <div className="relative flex-1 h-6 mr-3">
          <input
            type="text"
