...
import { X, PaperPlaneTilt, Gear, OpenAiLogo, Spinner, Trash, Plus, Sparkle, ChatCircle } from '@phosphor-icons/react';
...
const PROVIDERS = [
  ...
  { 
    ...
    icon: (props: any) => <svg fillRule="evenodd" style={{ flex: 'none', lineHeight: 1 }} viewBox="0 0 24 24" fill="currentColor" {...props}><path d="M13.827 3.52h3.603L24 20h-3.603l-6.57-16.48zm-7.258 0h3.767L16.906 20h-3.674l-1.343-3.461H5.017l-1.344 3.46H0L6.57 3.522zm4.132 9.959L8.453 7.687 6.205 13.48H10.7z"/></svg>
  },
  { 
    id: 'groq', 
    label: 'Groq', 
...
export const AIChatPanel: React.FC<{ 
...
}> = ({ onClose, isEmbedded, newMessage, onLoadingChange }) => {
  ...
  const sendMessage = async (messageText: string) => {
    ...
    const systemPrompt = `You are an expert database architect and SQL assistant. You help users design, optimize, and understand database schemas. Be concise and precise. Use markdown for formatting.${schemaContext}`;
  ...
  return (
    <div className={`${isEmbedded ? 'w-full h-full' : 'absolute left-3 top-14 bottom-14 w-[380px] rounded-lg border border-floating-border shadow-2xl animate-fade-in'} bg-floating-bg flex flex-col z-30`}>
      {/* Header */}
      <div className="relative flex items-center justify-between px-4 py-1.5 border-b border-border flex-shrink-0 min-h-[44px]">
        <div className="flex items-center gap-2">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1172.25 1172.249948" className="w-4 h-4 fill-primary">
        ...
        <div className="absolute left-1/2 -translate-x-1/2 flex p-0.5 bg-secondary/50 rounded-lg border border-border">
          <TooltipProvider delayDuration={200}>
            <Tooltip>
              ...
              <TooltipContent side="bottom" className="text-xs bg-black text-white border-0 px-2 py-1 font-medium shadow-lg">
              ...
              <Select value={config.model} onValueChange={(val) => updateConfig({ model: val })}>
                ...
                <SelectContent className="rounded-xl border-floating-border bg-floating-bg shadow-xl ai-provider-select-content">
                  ...
                  {config.customModels?.map((m) => (
                    <SelectItem key={m} value={m} className="text-xs rounded-lg my-0.5">
                      {m}
                    </SelectItem>
                  ))}
                </SelectContent>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            >
              {msg.role === 'assistant' ? (
                <div className="prose prose-sm prose-invert max-w-none [&_p]:m-0 [&_pre]:bg-card [&_pre]:p-2 [&_pre]:rounded [&_code]:text-xs">
